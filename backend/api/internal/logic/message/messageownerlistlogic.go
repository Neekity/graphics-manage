...
func (l *MessageOwnerListLogic) MessageOwnerList(req types.SearchMessageRequest, userId string) (*types.ApiResponse, error) {
	messages, err := l.svcCtx.GraphicsMessageModel.OwnerList(req.Title, req.ChannelId)
	if err != nil {
		return (*types.ApiResponse)(helper.ApiError(err.Error(), nil)), nil
	}
	messages, err = l.svcCtx.GraphicsMessageModel.FilterMessage(l.svcCtx.GraphicsCasbinRuleEnforce, messages, userId)
	if err != nil {
		return (*types.ApiResponse)(helper.ApiError(err.Error(), nil)), nil
	}
